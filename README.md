...
- **Guard** — `guard()` is a pre-execution safety check that runs *between* "model chose a tool" and "action fires." For any tool the manifest flags high-risk, it re-derives the money-critical facts from the user's own words and blocks on mismatch. Current gate coverage: the **money amount** (spoken or written — "forty dollars", "$54.94", cents-vs-dollars bugs) and the **named recipient/customer** ("to Jane" vs. a call targeting John), across common field aliases (`amount`/`amount_cents`, `customer`/`recipient`/`to`/`destination`).
...
| `node gate-benchmark.js` | Measured before/after for the guard: 4/4 wrong-money actions blocked, 0 false blocks on correct actions. |
...
- **Gate coverage.** The pre-execution guard currently re-derives and checks two money-critical facts from the user's words: the **amount** and the **named recipient/customer**. That's deliberately narrow and high-precision (0 false blocks on the benchmark's correct actions); extending it to more parameter families is straightforward and additive.
